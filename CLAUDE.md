...
`scripts/fetch_macro.py` (stdlib only) fetches 20 FRED series + `SP500` (200-DMA / 52-week deviation), computes week-over-week deltas vs the prior-run date, and degrades gracefully per series (`ok` / `derived` / `fetch_failed`). It also emits non-FRED blocks: `cftc_lev_funds` (leveraged-fund net position in UST futures, from CFTC's keyless weekly file + current-year history zip), `move_index` (^MOVE via Yahoo's chart JSON endpoint — unofficial, may break), `ofr_repo` (OFR STFM tri-party repo volume, keyless), and a derived `repo_stress` block (SOFR−IORB / SOFR99−IORB spreads, each leg pair on a shared date with offsets disclosed via `iorb_date`/`sofr99_date`/`sofr99_iorb_date`, + SRF usage `RPONTTLD`). When the prior-run date and the latest valid observation coincide (holiday/weekend runs), it emits `no_new_obs: true` with zero deltas instead of dropping the delta. Usage: `python3 scripts/fetch_macro.py <prior-run-date|none>`.
