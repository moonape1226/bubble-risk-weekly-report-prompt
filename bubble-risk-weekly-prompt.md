...
| | 項目 |
|---|---|
| **Best-effort**（可 `✗ NOT DISCLOSED`） | analyst TP upgrade decomposition；AI token volume growth；hyperscaler 客戶集中度／backlog 重複計算風險；OpenAI / Anthropic 營收；AI compute 供需／過剩；AI 營收對 capex 缺口；折舊年限變動；資本週期階段；PBoC aggregate financing；非美槓桿核准（KRX / TWSE / JPX / ESMA）；upcoming AI IPO timing；AI infrastructure debt financing / vendor-financing loops；private-credit / 非銀基金贖回壓力；BofA / JPM 機構調查（月頻）；社交情緒代理（Reddit / X）；NAAIM Exposure Index（週頻）；Cboe equity put/call ratio；CME FedWatch 隱含路徑；美債基差交易槓桿（CFTC 淨空倉 / MOVE / repo stress，script 優先）；美債標售需求（auction tail / bid-to-cover）；hyperscaler 融資結構（capex vs FCF / 發債）；AI 信用定價分歧（equity-vs-credit schism） |
| **Required**（須 `✓ API/DIRECT/SEARCH-VERIFIED` 或 `⛔ FETCH FAILED`，不得 `✗`） | 其餘全部——含 FRED 全序列（DGS10 / DFII10 / T10YIE / BAMLH0A0HYM2 HY OAS / BAMLC0A0CM IG OAS / DFEDTARU·DFEDTARL / WALCL / DCOILWTICO / CPIAUCSL CPI YoY / T5YIFR / THREEFYTP10 期限溢價 / SOFR·SOFR99·IORB·RPONTTLD repo 資金壓力與 SRF 動用 / LNFACBW027SBOG 銀行對 NBFI 放款）、S&P 500 P/E・CAPE、Mag 7 P/E、RSP/SPY 廣度、Top-10 集中度、A/D、CNN F&G、Margin Debt、AAII、0DTE / options volume、美國槓桿 ETF AUM、VIX / SKEW、IPO heat、+AI rename / SPAC 掃描、microcap moonshots、insider Form 4、ECB・BOJ、S&P 500 趨勢偏離（200-DMA/52週MA，由 SP500 計算）、家庭持股佔金融資產比（BOGZ1FL153064486Q） |
...
- repo 資金壓力（SOFR−IORB）與 SRF 動用: FRED series SOFR, SOFR99, IORB, RPONTTLD (all daily; required, same-class combined row; surfaced via the script's `repo_stress` block; each leg uses the latest valid observation at or before its reference date — when the two legs' dates differ, the offset is disclosed via `iorb_date` / `sofr99_date` / `sofr99_iorb_date` and must not be presented as a same-day spread) — SOFR−IORB 與 99th 分位尾部利差量測 secured-funding 壓力；RPONTTLD 為 Fed 隔夜 repo 操作總量（全擔保品，Treasury＋agency＋MBS），作 SRF 動用 read。判讀注意：2025-12 FOMC 取消 SRF $500bn 總量上限後，季末／稅期的例行動用本身不是失序訊號，需併同 SOFR−IORB 走闊與基差平倉新聞判讀
...
- **資本週期階段（capital cycle stage）[primary: SEARCH]** (best-effort; quarterly stock-of-state + 30-day event scan): 供給增速對需求增速的週期定位。供給側 proxies：top-5 hyperscaler ＋ neocloud 合計 capex 年增率、資料中心容量（GW）新增、GPU 出貨量估計；需求側 proxies：token volume 成長（既有 bullet）、AI 營收成長——僅取披露自身所載的成長軌跡（同一披露內的前值或明示成長率，如 run-rate $Xbn → $Ybn；只有單點水位時該 proxy 缺席，不得跨期自建基線）、利用率證據（AI compute supply/demand bullet）。增速對比僅在供給側與至少一個需求側 proxy 皆有當期可引證增速時輸出方向判讀；任一側增速證據不足時明標「增速證據不足，本期不判定週期階段」，不得推測補齊。並掃描 30 日內的週期進出場事件：新 neocloud 進入者／大型融資、資料中心租約取消或延後、專案縮減／退出。判讀方向：供給增速 > 需求增速且進入者仍湧入＝資本週期過熱段（高報酬吸引資本 → 產能擴張 → 報酬崩落的中段）；取消／延後事件密集＝週期轉折訊號。Distinct from the AI compute supply/demand bullet——該 bullet 追當期價格／利用率證據（缺口是否已現），本 bullet 追增速對比與進出場事件（週期定位）。If no current evidence, mark ✗ NOT DISCLOSED.
...
**Pre-archive validation gate (deterministic, required):** before the archive write, save the complete report markdown to a local temp file (a local validation copy is not an archive write and is permitted), then run `python3 scripts/validate_report.py <temp-file> --prompt <prompt-file>`, where `<prompt-file>` is a local copy of this prompt file `bubble-risk-weekly-prompt.md` (both the script and the prompt file: use the copy in the prompt repo's checkout if present on disk; if absent, fetch from raw.githubusercontent the same way as `fetch_macro.py`). `--prompt` makes the validator count the top-level `# Data sources` bullets and enforce that the Coverage table has exactly that many rows — do not omit it, and do not substitute a hand-typed `--coverage-rows` number. Fix every reported failure and re-run until it exits 0; only then invoke the connector write. In dry-run mode, still run the validator and print its result. The validator enforces the locked skeleton (12 sections, exact headings and §1/§2/§3 columns), the exact final disclaimer line, the score-JSON schema / weighted-total arithmetic / tier mapping / date-weekday-ISO-week consistency (the score block must parse to a JSON object — `null` or a wrong shape is a failure), §1/§2 bar-chart-vs-score consistency, the §2 five locked anchor rows, the §3 three indicator rows and its 結論 trigger-state line, the 總評 line and its cross-checks (Δ vs the §1 加權總分 row, 扳機狀態 vs the §3 結論), and the Coverage row count — it complements the internal self-check, it does not replace it. If `python3` is unavailable *and* the script cannot be fetched, state that the validation gate could not run at the end of the report and proceed with extra care on the self-check.
...
- 判定：{real-rate-driven / breakeven-driven / mixed / 無變動（無新觀測） / 不可判（視窗不一致）}（依 ΔDFII10 與 ΔT10YIE 何者主導；script `decomposition.driver == "none"` 時填 無變動（無新觀測）、`"unknown"` 時填 不可判（視窗不一致））
...
- **初啟**＝未達已擊發，但任一成立：breakeven 主導的 10Y 上行且 WTI 同步上行；HY OAS 連續兩次運行走闊；D5 rationale 本次標「扳機側」；槓桿鏈積累證據——CFTC 槓桿基金美債期貨淨空倉處於報導的紀錄／循環極端，且 MOVE 本週明顯上行；財政再定價併發 funding 壓力——ΔTHREEFYTP10 ≥ +15 bps（取 script `delta_bps`，序列自身 trailing ≈7d；calibration knob）且（MOVE 週升或 `sofr_iorb_bps` ≥ 0）。
