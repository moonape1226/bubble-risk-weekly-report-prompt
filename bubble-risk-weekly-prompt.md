...
- **AI 營收對 capex 缺口（revenue-to-capex gap）[primary: SEARCH]** (best-effort; quarterly stock-of-state): 產業層級的 ROI 直接量測。彙整已披露的 AI 年化終端營收（OpenAI / Anthropic run-rate、hyperscaler AI 營收敘述——取自本節既有 bullets，不另抓；Nvidia data-center 營收僅作供給側對照、不計入終端營收）對比 top-5 hyperscaler ＋大型 neocloud 的年化 AI capex（取自 hyperscaler capex guidance bullet）。輸出一句量級判讀，如「年化 AI 終端營收 ~$Xbn vs 年化 capex ~$Ybn，缺口擴大／收斂」。組件多為季頻或不定期披露：沿用最近可得值並逐組件標註資料日期；若本期無任何新組件披露，mark ✗ NOT DISCLOSED，沿用值僅作背景、不列 `## 本次新增訊號`。
- **GPU / 伺服器折舊年限變動（depreciation useful-life change）[primary: SEARCH]** (best-effort; event scan): scan 10-K / 10-Q / earnings calls（Microsoft、Alphabet、Amazon、Meta、Oracle、CoreWeave、大型 neocloud）for changes to server / GPU depreciation useful life 或殘值假設。年限延長＝壓低當期折舊、美化 E 與 FCF——光纖時代的經典盈餘品質訊號（報酬下滑被會計處理掩蓋）；年限縮短＝承認技術汰舊加速、capex 回收期惡化。記錄公司、變動前後年限、生效期、披露的 EPS 影響、來源 URL。If no change disclosed in the current window, mark ✗ NOT DISCLOSED.
- **資本週期階段（capital cycle stage）[primary: SEARCH]** (best-effort; quarterly stock-of-state + 30-day event scan): 供給增速對需求增速的週期定位。供給側 proxies：top-5 hyperscaler ＋ neocloud 合計 capex 年增率、資料中心容量（GW）新增、GPU 出貨量估計；需求側 proxies：token volume 成長、AI 營收成長、利用率證據（取自本節與 AI compute supply/demand bullet 既有抓取，不另抓）。並掃描 30 日內的週期進出場事件：新 neocloud 進入者／大型融資、資料中心租約取消或延後、專案縮減／退出。判讀方向：供給增速 > 需求增速且進入者仍湧入＝資本週期過熱段（高報酬吸引資本 → 產能擴張 → 報酬崩落的中段）；取消／延後事件密集＝週期轉折訊號。Distinct from the AI compute supply/demand bullet——該 bullet 追當期價格／利用率證據（缺口是否已現），本 bullet 追增速對比與進出場事件（週期定位）。If no current evidence, mark ✗ NOT DISCLOSED.
...
- **AI 營收對 capex 缺口 reality check**: 產業年化 AI 終端營收對年化 capex 的量級對比（Data sources 對應 bullet）。缺口持續擴大而 capex 指引仍上修＝回本假設後移、估值對「未來需求兌現」的依賴加深，屬估值風險上修的質化依據；缺口收斂＝基本面追上敘事。有當期披露時納入計分判讀；缺值（✗ NOT DISCLOSED）不調分。
