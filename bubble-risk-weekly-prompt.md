...
- repo 資金壓力（SOFR−IORB）與 SRF 動用: FRED series SOFR, SOFR99, IORB, RPONTTLD (all daily; required, same-class combined row; surfaced via the script's `repo_stress` block, IORB / SOFR99 aligned to the SOFR observation date) — SOFR−IORB 與 99th 分位尾部利差量測 secured-funding 壓力；RPONTTLD 為 Fed 隔夜 repo 操作總量（全擔保品，Treasury＋agency＋MBS），作 SRF 動用 read。判讀注意：2025-12 FOMC 取消 SRF $500bn 總量上限後，季末／稅期的例行動用本身不是失序訊號，需併同 SOFR−IORB 走闊與基差平倉新聞判讀
